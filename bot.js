...
client.on('presenceUpdate', async (oldPresence, newPresence) => {
  ...
  if (
    newPresence.member.presence.activities.length == 0 ||
    (newPresence.member.presence.activities.length == 1 &&
      newPresence.member.presence.activities[0].type == 'CUSTOM')
  ) {
  ...
  if (
    newPresence.member.presence.activities.length > 0 &&
    newPresence.member.presence.activities[0].type == 'STREAMING'
  ) {
    ...
    if (userStreaming == null) {
      await redisClient.setEx(
        username,
        DEFAULT_EXPIRATION,
        newPresence.member.presence.activities[0].state
      );
      ...
      const {
        ...
        url: activityUrl,
        details,
      } = newPresence.member.presence.activities[0];
        ...
        if (Id.length > 0) {
          getGameArt(Id[0].id).then((coverData) => {
            ...
            const streamEmbed = new MessageEmbed()
              .setTitle(details)
              .addFields({ name: 'Peli', value: `${gameName}` })
              .setAuthor({
                name: `${newPresence.user.username}`,
                iconURL: `${newPresence.user.avatarURL()}`,
              })
              .setImage(url)
              .setTimestamp();
...
client.on('messageCreate', (msg) => {
  ...
  if (msg.channel.id === process.env.STRIIMI_KANAVA_ID) {
    if (containsUrl(msg.content) && !containsTwitchUrl(msg.content)) {
      msg.channel.send(
        `${msg.author} Anteeksi, mutta tämä tekstikanava sallii vain Twitch-linkit`
      );
      msg.delete();
    }
  }
  ...
  if (command.startsWith(PREFIX)) {
    switch (command.toLowerCase()) {
          ...
          .addFields(
            {
              name: '!twitch',
              value:
                'Lähettää kanavalle Twitch-linkin striimiisi. Esim. !twitch tartaroksentatit',
            },
            {
              name: '!komennot',
              value: 'Jos nyt on pakko tämä kertoa, niin luet tätä jo!',
            }
          )
          .setTimestamp()
      ...
      default:
        msg.reply(
          `Antamaasi komentoa ${command} ei löydy. Tarkista komennot kirjoittamalla !komennot`
        );
    }
  }
});
